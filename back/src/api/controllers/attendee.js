...
const deleteAttendanceToAnEvent = async (req, res, next) => {
    try {
        ...
        if (!eventAttendee) {
            const error = new Error('El asistente no ha confirmado asistencia a este evento previamente');
            error.statusCode = 400;
            return next(error);
        }
        ...
        const updatedAttendee = await Attendee.findByIdAndUpdate(attendee._id, { $pull: { attended_events: { _id: eventId } } });
        ...
        if (attendee.attended_events.length === 0) {
        ...
        // Devolver una respuesta exitosa con la información del asistente actualizada o eliminada, según sea el caso
